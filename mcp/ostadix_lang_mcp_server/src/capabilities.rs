...
Prefer o_execute with exactly one source or path for a complete polyglot computation. action defaults to execute; check parses without executing, plan inspects statically, and compile requests a compiler artifact. Keep cwd explicit when relative filesystem context matters. mode admitted automatically binds ordinary source and graph intent before fresh native admission. Existing files and linked projects remain first-class inputs.
placement auto uses local HGraph for ordinary O and native mesh-prefer for projects. placement local requests local execution; mesh-required requires supported project placement and never silently becomes local success. Project results retain native run identities and evidence; local runtime results expose the existing O JSON value/error object. Neither a plan nor a started job proves execution success.
Use o_env, o_runtimes and o_doctor when diagnosing the installation. Discover expert capabilities by family, then inspect explicit supported help through o_cli. Catalog descriptions are source-backed; a located executable may be older and does not prove runtime readiness. No discovery call starts daemons or probes optional services.
Use route for explicit project execute/plan/IR/DOT selection. Use node to send a complete ordinary document to one authenticated peer through native octl, without local fallback or automatic retries. Node cwd/env configure the local client only, and cancelling it cannot stop remote effects. This does not partition an ordinary graph.
Use o_cli for expert project/route-policy/retained-evidence options, compiler controls, packaging, hosted-session administration, Worlds, signed records, O-core, boot media and absorbed systems. o_eval and legacy tools remain compatible.
...
The primary MCP call is o_execute {source: COMPLETE_O_SOURCE}, or {path: EXISTING_PROGRAM}. Use action check for parse-only validation and mode admitted for automatic same-source/same-intent binding before native admission. Structured results preserve native O JSON and job logs. Check does not prove type, grounding, backend health or admission. Use o_eval/o_cli for specialized interpreter flags or PTYs.
...
Use o_execute with source or path and action plan for static inspection; action compile with target and output requests an artifact. Inline compiler input is materialized privately by the server, preserving cwd and source lifetime. Native script target executes code, so use execution or the explicit expert compiler interface for it.
...
o_execute accepts existing project directories and linked project bundles, including complete inline bundle source. Default placement auto delegates to the native mesh-prefer policy; local requests local dispatch and mesh-required retains native rejection when remote placement cannot be satisfied. Native project summaries and retained run references remain evidence, not a claim that every operation ran remotely. Set route for an explicit native route or route-set in execute/plan/IR/DOT. Marked operations retain planner-owned route selection and reject overrides. Binary/Wasm artifacts select routes at runtime. Use o_cli for route policies and marked-operation options.
...
o_execute with node sends a complete ordinary source/path document via native octl node run. It requires direct execution, rejects explicit placement/workers/stdin/project routes and uses the native hosted V1 contract. Foreground results preserve the digest-bound receipt and typed value; background receipts remain in job logs. Local client cancellation cannot cancel remote effects. Omitted/zero timeout preserves native IO 60s/publication 300s; positive values are capped at IO 3600s/publication 86400s. Hosted V1 source/outcome bounds remain native, and this is distinct from project mesh or ordinary graph partitioning.
For normal project computation use o_execute {path: PROJECT, placement: auto|local|mesh-required}; node administration is not required tool choreography. Ordinary inline O has local HGraph execution and cannot request arbitrary remote graph partitioning. Native placement validation and configured peer/authority state remain decisive.
