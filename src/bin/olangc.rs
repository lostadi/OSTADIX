...
fn generate_cargo_lock_from(workspace_lock: &str, include_project: bool) -> Result<String> {
    ...
    strip_unused_workspace_feature_edges(&mut lock)?;
...
fn strip_unused_workspace_feature_edges(lock: &mut toml::Value) -> Result<()> {
    let packages = lock
        .get_mut("package")
        .and_then(toml::Value::as_array_mut)
        .context("workspace Cargo.lock has no mutable package array")?;
    for package in packages.iter_mut().filter_map(toml::Value::as_table_mut) {
        ...
        for &(target_package, excluded_dependency) in GENERATED_RUNTIME_UNUSED_FEATURE_EDGES {
...
mod tests {
    ...
    fn generated_runtime_includes_hgraph_modules() {
        ...
        for path in [
            ...
            "hosted_remote/fabric/client.rs",
            "hosted_remote/fabric/coordinator.rs",
            "hosted_remote/fabric/keys.rs",
            "hosted_remote/fabric/ledger.rs",
            "hosted_remote/fabric/profile.rs",
        ...
        for include_project in [false, true] {
            for bin_name in ["generated-runtime", "serde"] {
                ...
                assert!(!packages
                    .iter()
                    .filter_map(toml::Value::as_table)
                    .any(|package| package.get("name").and_then(toml::Value::as_str)
                        == Some("spake2")));
            }
        }
    }
