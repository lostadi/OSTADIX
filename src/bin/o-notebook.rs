...
function createCell(code = '') {
  ...
  div.innerHTML = `
    <div class="ct">
      ...
      <button class="rbtn" aria-label="Run cell" id="rb${id}" onclick="runCell(${id})" title="Run (Shift+Enter)">▶</button>
      <span class="lang-badge">O</span>
      <div class="ca">
        <button class="cbtn" aria-label="Move cell up" onclick="moveCell(${id},-1)" title="Move up">↑</button>
        <button class="cbtn" aria-label="Move cell down" onclick="moveCell(${id},1)"  title="Move down">↓</button>
        <button class="cbtn" aria-label="Clear cell output" onclick="clearCell(${id})"  title="Clear output">⬜</button>
        <button class="cbtn del" aria-label="Delete cell" onclick="deleteCell(${id})" title="Delete">✕</button>
      </div>
    </div>
