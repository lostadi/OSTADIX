import { runOlangBrowserBundle } from "./linux-runner.mjs";
...
runButton.addEventListener("click", async () => {
  ...
  output.textContent = "Booting embedded Linux in a WebAssembly worker…";
  ...
  try {
    ...
    output.textContent = [
      `status: ${status}`, `exit: ${result.exitCode}`,
    ...
    document.dispatchEvent(new CustomEvent("olang-browser-run-complete", { detail: Object.freeze(completion) }));
  }
});
